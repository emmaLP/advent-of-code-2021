...
func main() {
	...
	var boardsInWinningOrder []NumGrid
	for bingoNumIndex, num := range bingNumbers {
		number, _ := strconv.Atoi(num)
		fmt.Println("Drawn number:", number)
		for _, grid := range grids {
			if checkForBingo(grid.grid) {
				fmt.Println("Board is already complete, ignoring")
				continue
			}
			...
			if checkForBingo(grid.grid) {
				grid.lastCallout = number
				boardsInWinningOrder = append(boardsInWinningOrder, grid)
			}
		}
	}
...
func checkForBingo(grid [][]int) bool {
	...
	for i := 0; i < len(grid); i++ {
		...
		if isBingo = reflect.DeepEqual(grid[i], []int{-1, -1, -1, -1, -1}); isBingo {
			fmt.Println("Row matched got bingo")
			return isBingo
		}
